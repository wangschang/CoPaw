...
# 安装 Python 包（如已安装 npm，前端控制台将自动构建）
pip install -e .
```

> **Web 界面说明：** 只要已安装 [Node.js / npm](https://nodejs.org/)，`pip install -e .` 会
> 自动在 `console/` 目录执行 `npm ci && npm run build`，并将产物复制到 `src/copaw/console/`。
> 若未检测到 npm，Python 包仍可正常安装，但 Web 控制台将不可用。可在稍后手动构建：
> ```bash
